...
from ..spleeter_types import Signal
...
class FFMPEGProcessAudioAdapter(AudioAdapter):
    ...
    SUPPORTED_CODECS: Dict[Codec, str] = {  # type: ignore
        Codec.M4A: "aac",
        Codec.OGG: "libvorbis",
        Codec.WMA: "wmav2",
    }
    ...
    def load(
        ...
        if not waveform.dtype == np.dtype(dtype.decode()):
            waveform = waveform.astype(dtype.decode())
        return (waveform, float(sample_rate) if sample_rate is not None else 44100.0)
